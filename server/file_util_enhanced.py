...
SUMMARIZED_FILES_DIR = "summarized_files"
...
class FileManager:
    ...
    def load_edited_file_or_parsed_file(self, filename: str) -> Tuple[str, dict]:
        """
        Load the edited file if it exists, otherwise load the parsed file.
        ...
        Raises:
            FileNotFoundError: If neither edited nor parsed file exists
        """
        # Ensure filename has .md extension
        base_name = filename.split(".")[0] if "." in filename else filename
        md_filename = f"{base_name}.md"
        
        # Try edited file first
        if self.file_exists(EDITED_FILES_DIR, md_filename):
            logger.info(f"Loading edited file: {md_filename}")
            return self.load_markdown_file(EDITED_FILES_DIR, md_filename)
        
        # Fall back to parsed file
        ...
        raise FileNotFoundError(
            f"Neither edited file nor parsed file exists for: {base_name}"
        )
